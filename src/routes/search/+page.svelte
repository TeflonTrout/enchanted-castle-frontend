<script lang='ts'>
  ...
  import type { ChangeEventHandler } from "svelte/elements";
    ...
    const updateSearchPage = async(newPage:number, newLimit:number) => {
        cardArray = []
        await axios.get(`https://enchanted-castle-server.onrender.com/all?sort=${sortBy}&page=${newPage}&limit=${newLimit}${queryString}`)
        .then(res => {
    ...
    const updateSearch = async(e:Event, newPage:number, newLimit:number) => {
        ...
        e.preventDefault()
        cardArray = []
        await axios.get(`https://enchanted-castle-server.onrender.com/all?sort=${target.value}&page=${newPage}&limit=${newLimit}${queryString}`)
        .then(res => {
                limitArr = Array.from({ length: limit }, (_, index) => index + 1)
                searchPage = res.data.page
                const length = res.data.totalPages
