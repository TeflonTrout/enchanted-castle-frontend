<script lang="ts">
    ...
    onMount(() => {
        axios.get(`https://enchanted-castle-api.onrender.com/cards/${data.setCode}/${data.cardNumber}`)
        .then(res => {
            cardData = res.data.data
            console.log(res.data.data)
        })
        for (let i = -1; i > -3; i--) {
            axios.get(`https://enchanted-castle-api.onrender.com/cards/${data.setCode}/${Number(data.cardNumber) + i}`)
            .then(res => {
                console.log(res.data.data)
